...
// Таблица инвентаря в моноширинном блоке
...
// ── HuggingFace Image (FLUX.1-schnell) ───────────────────────
async function generateImage(prompt) {
  ...
  if (!response.ok) {
    const err = await response.text();
    throw new Error(`Image API ошибка ${response.status}: ${err}`);
  }
  // Возвращает бинарные данные (image/jpeg или image/png)
  const buffer = Buffer.from(await response.arrayBuffer());
  return buffer;
}

// ── HuggingFace TTS (facebook/mms-tts-rus) ───────────────────
async function generateTTS(text) {
  const response = await fetch('https://router.huggingface.co/hf-inference/models/facebook/mms-tts-rus', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${QWEN_API_KEY}`,
  ...
  if (!response.ok) {
    const err = await response.text();
    throw new Error(`TTS API ошибка ${response.status}: ${err}`);
  }
  const buffer = Buffer.from(await response.arrayBuffer());
  return buffer;
}

// ── HuggingFace Video (Wan-AI/Wan2.1-T2V-14B) ────────────────
async function generateVideo(prompt) {
  // Шаг 1: запустить задачу
  const startRes = await fetch('https://router.huggingface.co/hf-inference/models/Wan-AI/Wan2.1-T2V-14B', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${QWEN_API_KEY}`,
  ...
  if (!startRes.ok) {
    ...
    throw new Error(`Video API ошибка ${startRes.status}: ${err}`);
  }
  const buffer = Buffer.from(await startRes.arrayBuffer());
  return buffer;
}
...
// In-memory кулдауны для шахты и экзамена
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'help_v2_0') {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '🤖 AI Советник',     value: '`/ask-ai` — текст / 🖼️ фото / 🔊 голос / 🎬 видео\nНачни запрос с `серьёзно` — без партийного пафоса', inline: false },
        );
    ...
    if (interaction.commandName === 'mine') {
      ...
      // Заработок: рандом 50–150 * уровень кирки
    ...
    if (interaction.commandName === 'ah_buy') {
      ...
      // Достижение за первую покупку на аукционе
    ...
    if (interaction.commandName === 'buy_v2_0') {
      ...
      if (item === 'cat_wife') {
        ...
        // Старая система — редирект на новую
    ...
    if (interaction.commandName === 'ask-ai') {
      if (!QWEN_API_KEY) {
        return interaction.reply({ content: '❌ API ключ не настроен (`QWEN_API_KEY`).', flags: 64 });
      }
      ...
      try {
        ...
        return interaction.editReply({ content: `❌ ИИ недоступен: ${err.message}` });
      }
    }
  ...
  if (interaction.isButton()) {
    ...
    if (id.startsWith('case_open_')) {
      ...
      // Достижение за легендарный предмет
      let achMsg = '';
    ...
    if (id.startsWith('trade_accept_')) {
      const parts     = id.split('_');
      // trade_accept_{sellerId}_{buyerId}_{itemId}_{quantity}_{price}
