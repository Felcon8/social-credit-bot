  ...
  const response = await fetch('https://api.deepseek.com/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'Ты — партийный советник Великой Партии. Отвечай с пафосом, патетикой и в духе социалистической риторики. Иногда хвали Партию. Будь немного абсурдным и шуточным. Отвечай на том языке, на котором к тебе обращаются.',
        },
...
async function registerCommands() {
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName('wheel_v2_0')
      .setDescription('Покрутить колесо фортуны (раз в сутки)'),
...
client.on('ready', () => {
  ...
  checkWorkerOfDayReset(client, GUILD_ID, WORKER_DAY_MS, WORKER_OF_DAY_BONUS_CREDITS, WORKER_OF_DAY_BONUS_YUAN)
...
client.on('interactionCreate', async interaction => {
 ...
 // ── /exam_v2_0 ──────────────────────────────────────────
  ...
  // ── /profession_v2_0 ────────────────────────────────────
    ...
    if (job === 'miner') {
      ...
      if (item.id === 'diamond') {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(`Ты нашёл АЛМАЗ! Это чудо!\n💎 ${item.name}\n💴 +${earn} юаней\n⭐ +5000 соц. кредитов\n🩹 Все травмы вылечены!`);
      ...
      // Шанс травмы
      if (item.injuryChance > 0 && Math.random() * 100 < item.injuryChance) {
        await setInjury(userId, INJURY_MS);
        const embed = new EmbedBuilder()
          .setColor(0xFF4500)
          .setTitle(`⛏️ Шахтёр — ${item.name} (с травмой!)`)
          .setDescription(`Ты добыл ${item.name}, но получил травму!\n💴 +${earn} юаней\n🩹 Лечение: **${formatTime(INJURY_MS)}**`);
        return interaction.reply({ embeds: [embed] });
      }

      const embed = new EmbedBuilder()
        .setColor(0x00FF88)
        .setTitle(`⛏️ Шахтёр — ${item.name}`)
    ...
    // ── Общий провал (бухгалтер или шпион без тюрьмы) ──
  ...
  // ── /buy_v2_0 ───────────────────────────────────────────
    ...
    if (item === 'cat_wife') {
      if (p.wallet < 50000)    return interaction.reply({ content: '❌ Недостаточно юаней! Нужно **50 000**.', flags: 64 });
      if (p.items.cat_wife)    return interaction.reply({ content: '❌ У тебя уже есть кошка-жена!', flags: 64 });
      p.wallet -= 50000;
      p.items.cat_wife = true;
      await p.save();
      ...
      if (p.wallet < 5000) return interaction.reply({ content: '❌ Недостаточно юаней! Нужно **5 000**.', flags: 64 });
      p.wallet -= 5000;
      p.items.rice_bowls += 1;
      await p.save();
      await interaction.reply({ content: `🍚 Ты купил **миску риса**! Теперь у тебя **${p.items.rice_bowls}** мисок.` });

    } else if (item === 'ticket') {
      if (p.wallet < 1000) return interaction.reply({ content: '❌ Недостаточно юаней! Нужно **1 000**.', flags: 64 });
      p.wallet -= 1000;
      await p.save();
