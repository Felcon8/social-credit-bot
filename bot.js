...
async function registerCommands() {
  const commands = [
   ...
   if (interaction.commandName === 'case') {
    const cost = 10000; // Цена кейса
    const p = await getPlayer(interaction.user.id);

    if (p.yuan < cost) {
        return interaction.reply({ content: `❌ Недостаточно юаней! Кейс стоит ${cost} юаней.`, ephemeral: true });
    }

    // Убираем юани
    await addYuan(interaction.user.id, -cost);

    // Логика лута (шансы)
    const items = ['double_mine', 'triple_mine', 'speed_boost'];
    const reward = items[Math.floor(Math.random() * items.length)];

    // Добавляем в инвентарь (пусть будет поле upgradesInventory)
    await Player.updateOne({ userId: interaction.user.id }, { $push: { upgradesInventory: reward } });

    await interaction.reply({ content: `📦 Вы открыли кейс и получили: **${reward}**!`, ephemeral: true });
}
    ...
    // /applyupgrade — установка улучшений
    new SlashCommandBuilder()
      .setName('applyupgrade')
      .setDescription('Установить купленное улучшение на кирку')
      .addStringOption(o => o.setName('upgrade')
        .setDescription('ID улучшения (например: triple_mine)')
        .setRequired(true)),
